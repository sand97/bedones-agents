...
export const optinConfig = () => ({
  ...
  // When a member writes to the CORE number and their last opt-in reply is older
  // than this many hours, we re-send the opt-in template (re-entry into the loop
  // without going through the dashboard). Kept below OPTIN_WINDOW (24h) so we ask
  // again before the window expires. Also used as the re-ask cooldown.
  reaskHours: Number(process.env.WHATSAPP_OPTIN_REASK_HOURS ?? '16'),
})
...
export type OptinTrigger = 'cron' | 'dashboard' | 'reask'
